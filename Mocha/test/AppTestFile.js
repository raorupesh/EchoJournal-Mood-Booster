...
describe('Journal Entry API Tests', function () {
    ...
    describe('GET /app/journal/:id/:userId', function () {
        ...
        it('Should return 404 for non-existent id/userId', function (done) {
            chai.request(BASE_URL)
                .get(`/app/journal/000000000000000000000000/${testUserId}`)
                .end(function (err, res) {
        ...
        it('Should return 404 for invalid id format', function (done) {
            chai.request(BASE_URL)
                .get(`/app/journal/123456678/${testUserId}`)
                .end(function (err, res) {
                    expect(res).to.have.status(404);
                    expect(res.body).to.have.property('success', false);
    ...
    describe('GET /app/journal/all/:userId', function () {
        ...
        it('Should support pagination with page and limit query', function (done) {
            chai.request(BASE_URL)
                .get(`/app/journal/all/${testUserId}?page=1&limit=1`)
                .end(function (err, res) {
                    expect(res).to.have.status(200);
                    expect(res.body).to.have.property('success', true);
                    expect(res.body).to.have.property('entries');
